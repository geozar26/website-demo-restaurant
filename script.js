...
// --- GSAP ANIMATIONS (SPEED & MOTION FIX) ---
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    gsap.from(".items .item", {
        scrollTrigger: { trigger: ".items", start: "top 90%" },
        opacity: 0,
        y: 15,
        duration: 0.4,
        stagger: 0.1,
    ...
    // 2. Testimonials - Γρήγορο & Σταδιακό (Stagger)
    gsap.from(".testimonial-card", {
        scrollTrigger: { trigger: ".testimonials-section", start: "top 90%" },
        opacity: 0,
        y: 15,            // Μικρή κίνηση για να μην βγάζει scrollbar
        duration: 0.3,    // Πιο γρήγορο duration
        stagger: 0.1,     // Μικρό stagger για να φαίνεται η διαδοχή γρήγορα
        ease: "power1.out"
    });
    ...
    // 4. Footer - Η κίνηση που ζήτησες (Up & Down)
    const footerTimeline = gsap.timeline({
        scrollTrigger: { trigger: ".footer", start: "top 95%" }
    });

    footerTimeline
        // Τα κείμενα των στηλών από κάτω προς τα πάνω
        .from(".footer-col", {
            opacity: 0,
            y: 20, 
            duration: 0.4,
        ...
        // Τα social icons από πάνω προς τα κάτω
        .from(".footer-left .social-icons a", {
            opacity: 0,
            y: -20, 
            duration: 0.3,
            stagger: 0.05,
            ease: "back.out(1.7)"
        }, "-=0.2") // Ξεκινάει πριν τελειώσουν οι στήλες
        // Το copyright εμφανίζεται ΑΜΕΣΩΣ μετά
        .from(".footer-bottom", {
            opacity: 0,
            duration: 0.3,
            ease: "none"
